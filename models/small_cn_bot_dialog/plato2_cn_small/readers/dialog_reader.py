...
from plato2_cn_small.utils import pad_batch_data
from plato2_cn_small.utils.args import str2bool
from plato2_cn_small.utils.masking import mask
import plato2_cn_small.utils.tokenization as tokenization
...
class DialogReader(object):
    ...
    def _convert_example_to_record(self, example, is_infer):
        ...
        for i, s_token_ids in enumerate(s_token_ids_list[idx + 1:], idx + 1):
            # this is changed, adding a dialog type label at the beginning.
            # Source code:
            ...
            ##########################################################
            if i == 0:
                src_token_ids += [30001] + s_token_ids
                src_pos_ids += list(range(1, len(s_token_ids) + 2))
            else:
                src_token_ids += s_token_ids
                src_pos_ids += list(range(1, len(s_token_ids) + 1))
            ##########################################################
